...
## 예시 - CPU 사용률이 100%일때 Load Average 값 별 내부 상태 및 예상 동작 
|부하 단계|Load Average 수준|내부 상태 (Internal State)|예상 동작 및 결과|권장 조치|
|------------------|------------|--------------|--------------|--------------|
|1단계: 포화|코어 수 x1|모든 코어가 작업 수행 중 (Idle 0%)|- 최적의 처리량(Throughput) 유지<br>- 지연 없음|모니터링 강화 및 추이 관찰|
|2단계: 경합|코어 수 x2|- 런큐(Run-queue) 대기 발생<br>- 타임 슬라이스 분할 시작|- 응답 지연(Latency) 2배 증가<br>- 서비스 중량감 감지|부하 원인 프로세스 최적화 검토|
|3단계: 임계|코어 수 x3|- 컨텍스트 스위칭 오버헤드 급증<br>- 인터럽트 처리 지연|- 패킷 드롭 <br> - 서비스 타임아웃 빈번 <br>- 가용성 저하|Scale-up 또는 Scale-out 즉시 실행|
|4단계: 고립|코어 수 x4 이상|- Livelock/Thrashing<br>- 커널 스케줄러 제어권 상실|- System Hang<br> - SSH 접속 불능<br>- Watchdog 재부팅|Serial Console 접속 또는 Hard Reset|